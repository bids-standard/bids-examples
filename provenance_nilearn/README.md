...
> [!WARNING]
> We don't know right now how to deal with the BIDS validator "NOT_INCLUDED" error that occurs on files
...
In version 0.12.0, Nilearn provides the following BIDS compliant provenance metadata inside the `dataset_description.json` generated by this example.
...
The provenance metadata we created for this example offers to be more precise while showing no regression with respect to this `GeneratedBy`. Indeed the information provided by every metadata field in the `GeneratedBy` can be retrieved or has an equivalent representation in this example:
* `CodeURL` corresponds to the `AltIdentifier` of the Nilearn software (Id `bids::prov#nilearn-JAk1fM3q`) described inside `prov/prov-nilearn_soft.json`
* `Description` corresponds to the `Description` of the `bids::prov#glm-TzAuB7k8` activity described inside `prov/prov-nilearn_act.json`
* `Name` corresponds to the `Label` of the `bids::prov#glm-TzAuB7k8` activity described inside `prov/prov-nilearn_act.json`
* `Version` corresponds to the `Version` of the Nilearn software (Id `bids::prov#nilearn-JAk1fM3q`) described inside `prov/prov-nilearn_soft.json`
